...
import { Search, Filter, User as UserIcon, Mail, Phone, Calendar, Shield, Eye, MoreVertical } from 'lucide-react';
...
export default function UserManagement() {
  ...
  const getRoleColor = (role) => {
    ...
    return colors[role?.toLowerCase()] || 'bg-slate-50 text-slate-600 border-slate-100';
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 shrink-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 leading-tight">Identity Management</h2>
              <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">{allUser?.length || 0} Registered Entities</p>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto space-y-6">
                ...
                <table className="w-full min-w-[900px]">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      <th className="text-left px-8 py-4 text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">User Entity</th>
                      <th className="text-center px-6 py-4 text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Access Role</th>
                      <th className="text-center px-6 py-4 text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Registration</th>
                      <th className="text-center px-6 py-4 text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Terminal Status</th>
                      <th className="text-center px-6 py-4 text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {displayedUsers.map((user) => (
                      <tr key={user._id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-6 py-5 text-center">
                          <span className={`inline-flex px-3 py-1 rounded-lg text-[0.65rem] font-black uppercase tracking-widest border ${getRoleColor(user.role)}`}>
                            {user.role}
                          </span>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <div className="text-sm font-bold text-slate-700">{new Date(user.createdAt).toLocaleDateString('en-IN')}</div>
                          <div className="text-[0.65rem] font-bold text-slate-400 uppercase">Registered Date</div>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[0.65rem] font-black uppercase tracking-widest ${
                            user.isActive 
                              ? "text-emerald-500 bg-emerald-50/50" 
                              : "text-rose-500 bg-rose-50/50"
                          }`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${user.isActive ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`}></div>
                      ...
                      <tr>
                        <td colSpan="5" className="py-20 text-center">
                          <div className="flex flex-col items-center justify-center text-slate-300 space-y-2">
                            ...
                            <div className="font-bold italic">No matching records found in the database</div>
                            <button onClick={handleClearFilters} className="text-xs font-black text-slate-900 uppercase tracking-widest underline decoration-2 underline-offset-4">Reset Search</button>
                          </div>
                        </td>
                      </tr>
                    )}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 px-2">
              <div className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest ordered-1">
                Displaying {startIdx + 1}-{Math.min(startIdx + usersPerPage, filteredUsers.length)} of {filteredUsers.length} Records
              </div>
              ...
              <div className="flex items-center gap-2 order-2">
                ...
                <div className="flex gap-1 h-9">
                  ...
                  {totalPages > 5 && <span className="flex items-end px-1 text-slate-400 font-black">...</span>}
                </div>
